...
object MealLibrary {
     ...
     */
    private val hostnameVerifier = HostnameVerifier { hostname, session -> true }
     ...
     * getKcalNew
     */
    fun getKcalNew(CountryCode: String, schulCode: String, schulCrseScCode: String,
                   schulKndScCode: String, schMmealScCode: String): Array<String?> {
    ...
    private fun getKcalSubNew(content: Array<String?>, url: String): Array<String?> {
        ...
        work@ for (i in table.indices) {
            if ((table[i] as Element).getAttributeValue("class") == "tbl_type3") {
                ...
                val __tr = (tbody[0] as Element).getAllElements("tr")
                 ...
                 */
                for (index in 42..45) {
                    val __th = (__tr[index] as Element).getAllElements("th")

                    if ((__th[0] as Element).content.toString() == "에너지(kcal)") {
                        val td = (__tr[index] as Element).getAllElements("td")
    ...
    private fun getPeopleSubNew(content: Array<String?>, url: String): Array<String?> {
        ...
        for (i in table.indices) {
            if ((table[i] as Element).getAttributeValue("class") == "tbl_type3") {
                val tbody = (table[i] as Element).getAllElements("tbody")
                val __tr = (tbody[0] as Element).getAllElements("tr")
                val __th = (__tr[0] as Element).getAllElements("th")

                if ((__th[0] as Element).content.toString() == "급식인원") {
                    val td = (__tr[0] as Element).getAllElements("td")
